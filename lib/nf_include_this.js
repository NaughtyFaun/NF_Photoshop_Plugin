...
function NFInitialize()
{
    ...
    /**
     * Search for the layer by its name in given context.<br/>
     * Default layers.getByName sucks, it throws when there's no layer was found
     * @param name layer name as string
     * @param layers array, use either Document.layers or LayerSet.layers. undefined will use active document layers.
     * @returns {ArtLayer|LayerSet|undefined}
     */
    app.nf.findLayerOrGroup = function(name, layers)
    {
        ...
        for(var i = 0; i < layers.length; i++)
        {
            ...
            {
                return app.nf.findLayerOrGroup(name, layers[i].layers)
            }
        }
    ...
    {
        return app.nf.findLayerOrGroup(name) != undefined
    }

    app.nf.debugMarkerAdd = function(name)
    {
        var marker = app.nf.findLayerOrGroup(name)
        if (marker != undefined)
        {
